...
; Syntax.........: _WinHttpSimpleFormFill($hInternet [, $sActionPage = Default [, $sFormId = Default [, $sFieldId1 = Default [, $sData1 = Default [, (...)]]]]])
; Parameters ....: $hInternet - Handle returned by _WinHttpConnect().
;                  $sActionPage -  [optional] path to the page with form (default: "" - empty string; meaning 'default' page on the server).
;                  $sFormId - [optional] Id of the form. Can be name too (read Remarks section).
...
; Return values .: Success - Returns HTML source of the page returned bay the server on submited form.
...
; Author ........: trancexx
; Modified.......:
; Remarks .......: $sFormId specifies Id of the form same as .getElementById(FormId). Aditionally you can use "name:FormName" to
;                  identify form by its name like with .getElementsByName(FormName). In this case FormName will be taken to be what's right of colon mark.
;                  In that case first form with that name is filled.
;                  This is not the case for $sFieldId. If "name:InputName" option s used all the fields with that name are filled with specified $sData data.
;                  This function can be used to fill forms with up to 40 fields at once. Unset fields are left default.
; Related .......: _WinHttpConnect
; Link ..........:
; Example .......:
;============================================================================================
Func _WinHttpSimpleFormFill($hInternet, $sActionPage = Default, $sFormId = Default, $sFieldId1 = Default, $sData1 = Default, $sFieldId2 = Default, $sData2 = Default, $sFieldId3 = Default, $sData3 = Default, $sFieldId4 = Default, $sData4 = Default, $sFieldId5 = Default, $sData5 = Default, $sFieldId6 = Default, $sData6 = Default, $sFieldId7 = Default, $sData7 = Default, $sFieldId8 = Default, $sData8 = Default, $sFieldId9 = Default, $sData9 = Default, $sFieldId10 = Default, $sData10 = Default, _
	...
	Local $aForm = StringRegExp($sHTML, "(?si)<\h*form\h*(.*?)<\h*/form\h*>", 3)
	If @error Then Return SetError(1, 0, "") ; There are no forms available
	; Process input
	Local $fGetFormByName, $sFormName
	...
	Local $sAction, $sEnctype, $sName, $sMethod = "GET"
	...
	For $iFormOrdinal = 0 To UBound($aForm) - 1
		...
		$sAttributes = StringRegExp($sForm, "(.*?)>", 3)
		If Not @error Then $sAttributes = $sAttributes[0]
		$aAttributes = StringRegExp($sAttributes, '\h*([^=]+)\h*=\h*(?:"|)(.*?)(?:"| |\Z)', 3) ; e.g. method="post" or method=post
		...
		$fSend = True
		Local $aArray, $aSplit, $sInputId, $sInputName, $sInputValue, $sBoundary, $sPassedId, $fMuftiPart = False
		Switch $sEnctype
			Case "", "text/plain"
				$aInput = StringRegExp($sForm, "(?si)<\h*input\h*(.*?)/\h*>", 3)
				Local $aInputIds[3][UBound($aInput)]
				For $i = 0 To UBound($aInput) - 1
					$aArray = StringRegExp($aInput[$i], '(?i).*?id\h*=\h*"(.*?)"', 3)
					If Not @error Then $sInputId = $aArray[0]
					$aArray = StringRegExp($aInput[$i], '(?i).*?name\h*=\h*"(.*?)"', 3)
					If Not @error Then $sInputName = $aArray[0]
					$aArray = StringRegExp($aInput[$i], '(?i).*?value\h*=\h*"(.*?)"', 3)
					If Not @error Then $sInputValue = $aArray[0]
				...
				For $k = 1 To $iNumParams
					...
					If @error Then
						For $j = 0 To UBound($aInputIds, 2) - 1
							If $aInputIds[0][$j] = $sPassedId Then ; like .getElementById(InputId)
								$sAddData = StringReplace($sAddData, $aInputIds[1][$j] & "=" & $aInputIds[2][$j], $aInputIds[1][$j] & "=" & Eval("sData" & $k))
							EndIf
						Next
					Else
						If $aSplit[0] = "name" Then ; like .getElementsByName(InputName)
							$sAddData = StringRegExpReplace($sAddData, "(?i)(" & $aSplit[1] & "=).*?(&|\Z)", "$1" & Eval("sData" & $k) & "$2")
						Else
							For $j = 0 To UBound($aInputIds, 2) - 1
								$sPassedId = $aSplit[1]
								If $aInputIds[0][$j] = $sPassedId Then ; like .getElementById(InputId)
				...
				If $sMethod = "GET" Then
					...
					$sAction = $sCurrent & "/" & $sAction & "?" & $sAddData
			...
			Case "multipart/form-data"
				If $sMethod = "POST" Then ; can't be GET
					$fMuftiPart = True
					$aInput = StringRegExp($sForm, "(?si)<\h*input\h*(.*?)/\h*>", 3)
					Local $aMInputIds[3][UBound($aInput)]
							...
							For $j = 0 To UBound($aMInputIds, 2) - 1
								If $aMInputIds[0][$j] = $sPassedId Then ; like getElementById
									$sAddData = StringReplace($sAddData, _
											...
											'Content-Disposition: form-data; name="' & $aMInputIds[1][$j] & '"' & @CRLF & @CRLF & Eval("sData" & $k) & @CRLF)
								EndIf
							Next
						Else
							If $aSplit[0] = "name" Then ; like getElementsByName
								$sAddData = StringRegExpReplace($sAddData, '(?si)\Q' & 'Content-Disposition: form-data; name="' & $aSplit[1] & '"' & '\E\r\n\r\n.*?\r\n', _
										'Content-Disposition: form-data; name="' & $aSplit[1] & '"' & @CRLF & @CRLF & Eval("sData" & $k) & @CRLF);$1' & @CRLF & Eval("sData" & $k)); & @CRLF)
							Else
								For $j = 0 To UBound($aMInputIds, 2) - 1
									If $aMInputIds[0][$j] = $sPassedId Then ; like getElementById
										$sAddData = StringReplace($sAddData, _
	...
	; Send
	If $fSend Then
		Local $hRequest = _WinHttpOpenRequest($hInternet, $sMethod, $sAction)
		If $fMuftiPart Then
		...
		EndIf
		_WinHttpAddRequestHeaders($hRequest, "Accept: application/xml,application/xhtml+xml,text/html;q=0.9,text/plain;q=0.8,image/png,*/*;q=0.5")
	...
	Else
	EndIf
	; If here then there is no form on the page with specified attributes (name or id)
	Return SetError(3, 0, "")
...
Func _WinHttpSimpleRequest($hConnect, $sType = Default, $sPath = Default, $sReferrer = Default, $sData = Default, $sHeader = Default, $fGetHeaders = Default, $iMode = Default)
	; Author: ProgAndy
	If $sType = Default Or $sType = -1 Then $sReferrer = "GET"
